				 ...
				 struct stream *s, size_t *min_len)
{
	...
	if (IS_SUBTLV(exts, EXT_SRV6_ENDX_SID)) {
		...
		for (adj = (struct isis_srv6_endx_sid_subtlv *)
			...
			stream_put(s, &adj->value, IPV6_MAX_BYTELEN);
	...
	if (IS_SUBTLV(exts, EXT_SRV6_ENDX_SID)) {
		...
		for (lan = (struct isis_srv6_lan_endx_sid_subtlv *)
			...
			stream_put(s, &lan->value, IPV6_MAX_BYTELEN);
				    ...
				    void *dest, int indent)
{
	...
	append_item(&subtlvs->srv6_end_sids,
		    copy_item_srv6_end_sid((struct isis_item *)sid));
	return 0;
out:
	if (sid)
			       ...
			       struct stream *s)
{
	...
	if (router_cap->srv6_cap.enabled) {
		...
		/* Then add SR Algorithm if set as per RFC 9352 section #3 */
				     ...
				     int indent)
{
	...
	if (json) {
		...
		sbuf_push(
			buf, indent, "SRv6 Locator: %s (Metric: %u)%s",
			prefix2str(&loc->prefix, prefixbuf, sizeof(prefixbuf)),
			loc->metric,
			CHECK_FLAG(loc->flags, ISIS_TLV_SRV6_LOCATOR_FLAG_D)
				? " D-flag"
