	...
	if (!args->delete) {
		...
		if (args->segs) {
			...
			if (args->srv6_encap_source) {
				nb_cli_enqueue_change(vty, xpath_srv6_encap_source, NB_OP_MODIFY,
						      args->srv6_encap_source);
			} else {
				nb_cli_enqueue_change(vty, xpath_srv6_encap_source, NB_OP_DESTROY,
						      NULL);
			}
		} else {
			strlcpy(xpath_segs, xpath_nexthop, sizeof(xpath_segs));
			     ...
			     bool show_defaults)
{
	...
	if (yang_dnode_exists(nexthop, "./srv6-segs-stack/encap-source")) {
		yang_dnode_get_ipv6(&srv6_encap_source, nexthop, "./srv6-segs-stack/encap-source");
